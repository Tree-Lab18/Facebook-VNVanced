...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const expandIcon = document.getElementById('showMore')?.querySelector('svg');
    ...
    function isValidFacebookTab(url) {
        ...
        try {
            ...
            return hostname === 'facebook.com' || hostname.endsWith('.facebook.com') || hostname === 'messenger.com' || hostname.endsWith('.messenger.com');
        } catch (e) {
            return false;
        }
    }
    ...
    function addEventListeners() {
        ...
        if (donateLink && qrScanContainer) {
            ...
            document.addEventListener('click', (event) => {
                if (qrScanContainer.style.display === 'block' && !qrScanContainer.contains(event.target) && event.target !== donateLink) {
                    qrScanContainer.style.display = 'none';
                }
            });
        }
        if (showMoreButton && moreOptionsDiv) {
            showMoreButton.addEventListener('click', () => {
                ...
                showMoreButton.classList.toggle('expanded', isHidden);
            });
            showMoreButton.setAttribute('aria-expanded', moreOptionsDiv.style.display === "block");
            showMoreButton.classList.toggle('expanded', moreOptionsDiv.style.display === "block");
        }
        if (logoToggle && adBlockToggle) {
            logoToggle.addEventListener('click', () => {
                const newState = !adBlockToggle.checked;
                adBlockToggle.checked = newState;
                handleSettingChange('adBlockEnabled', newState, { action: 'toggleAdBlock', enabled: newState });
                updateIcon(newState);
            });
        }
        reelsToggle?.addEventListener('change', (e) => handleSettingChange('reelsEnabled', e.target.checked, { action: 'toggleReels', enabled: e.target.checked }));
        suggestToggle?.addEventListener('change', (e) => handleSettingChange('suggestEnabled', e.target.checked, { action: 'toggleSuggest', enabled: e.target.checked }));
        storyToggle?.addEventListener('change', (e) => handleSettingChange('storyEnabled', e.target.checked, { action: 'toggleStory', enabled: e.target.checked }));
        newFeedsToggle?.addEventListener('change', (e) => handleSettingChange('newFeedsEnabled', e.target.checked, { action: 'toggleNewFeeds', enabled: e.target.checked }));
        commentFilterToggle?.addEventListener('change', () => {
            ...
            sendMessageToContentScript({ action: 'toggleCommentFilter', enabled, filters });
        });
        ...
        commentFiltersTextarea?.addEventListener('blur', () => {
            ...
            if (commentFilterToggle?.checked) {
                sendMessageToContentScript({ action: 'toggleCommentFilter', enabled: true, filters: getCommentFilters() });
            }
        });
        compactUISelect?.addEventListener('change', (e) => handleSettingChange('compactUIValue', e.target.value, { action: 'setCompactUI', value: e.target.value }));
    }

    function loadSettingsAndCounts() {
        chrome.storage.sync.get(
            ['adBlockEnabled', 'reelsEnabled', 'suggestEnabled', 'storyEnabled', 'newFeedsEnabled', 'commentFilterEnabled', 'commentFilters', 'compactUIValue'],
            (data) => {
                if (chrome.runtime.lastError) {
                    return;
                }
                try {
                    ...
                    if (commentFiltersTextarea) {
                        ...
                        commentFiltersTextarea.value = (typeof data.commentFilters === 'string') ? data.commentFilters : defaultFilters;
                    }
                    if (compactUISelect) {
    ...
    function updateUICounts(adCount, spamCount) {
        adCountElement && (adCountElement.textContent = formatDisplayCount(adCount));
        spamCountElement && (spamCountElement.textContent = formatDisplayCount(spamCount));
    }
    ...
    function updateIcon(isEnabled) {
        const iconPaths = isEnabled
            ...
            : { "16": "icons/icon16_disabled.png", "48": "icons/icon48_disabled.png", "128": "icons/icon128_disabled.png" };
        try {
